...
SELECT `facid`, `name`, `membercost`, `monthlymaintenance` FROM `Facilities` WHERE `membercost` < (.2 * `monthlymaintenance`) and `membercost` > 0
...
SELECT distinct(f.name), case when b.memid > 0 then f.membercost else f.guestcost end as cost,
...
SELECT distinct(sub.f_name), sub.member, sub.cost

FROM ( 	select f.name as f_name, CONCAT(m.firstname, ' ', m.surname) as member, case when b.memid > 0 then f.membercost else f.guestcost end as cost
      	from `Bookings` as b 
